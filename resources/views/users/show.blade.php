...
@section('title')
    {{ $selectedUser->name . ' - ' . __('profile_info') }}
@endsection
                ...
                <div class="col-sm-9 page-content">
                    ...
                    <div class="inner-box">
                        <div class="welcome-msg">
                            <h3 class="page-sub-header2 clearfix no-padding">{{ strtoupper(__('slogan_p1')) }} <span>{{ __('slogan_p2') }} </span> {{ $selectedUser->name }}</h3>
                            <span class="page-sub-header-sub small">{{ __('last_login') . $diffTime }}</span>
                        </div>
                        <div id="accordion" class="panel-group">
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h4 class="panel-title"> <a href="#collapseB1" data-toggle="collapse"> {{ __('profile_info') }} </a> </h4>
                                </div>
                                    ...
                                    <div class="panel-body">
                                        ...
                                        {{ Form::model($selectedUser, ['route' => ['users.update', $selectedUser->id]]) }}
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div @if ($errors->has('address')) class="error-input" @endif>
                                                    {{ Form::text('address', null, ['class' => 'form-control', 'id' => 'address']) }}
                                                </div>
                                                <p> {{ $errors->first('address')}} </p>
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('avatar', __('update_avatar'), ['class' => 'control-label']) }}
                                            <br>
                                                {{ Form::file('avatar', ['id' => 'avatar', 'onchange' => 'readURL(this)']) }}
                                                <input type="button" value="{{ __('upload_avatar') }}" onclick="document.getElementById('avatar').click();" />
                                                <input type="button" id="cancel" value="{{ __('cancel_avatar') }}" onclick="backAvatar()" hidden="" />
                                                <input type="text" id="cancel-value" name="cancel_value" hidden="" value="cancel" disabled=""/>
                                                <input type="button" id="delete" name="delete" value="{{ __('delete_avatar') }}" onclick="deleteAvatar()"/>
                                            ...
                                            <br>
                                            <div class="form-group">
                                                {{ Form::label('detail', __('detail'), ['class' => 'control-label']) }}
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="form-inline">
                                                    {{ Form::select('province', $selectProvince, null, ['class' => 'form-control', 'id' => 'province', 'placeholder' => __('choose_province')]) }}
                                                    {{ Form::select('district', [], null, ['class' => 'form-control', 'id' => 'district', 'placeholder' => __('choose_district')]) }}
                                                    {{ Form::select('commune', [], null, ['class' => 'form-control', 'id' => 'commune', 'placeholder' => __('choose_commune')]) }}
                                                </div>
                                                <p> {{ $errors->first('working_place')}} </p>
                                            </div>
                                            @endif
                                            <div class="form-group">
                                                {{ Form::label('grade', __('grade'), ['class' => 'control-label']) }}
                                                <div @if ($errors->has('grade')) class="error-input" @endif>
                                                    {{ Form::text('grade', null, ['class' => 'form-control', 'id' => 'grade', 'readonly' => '']) }} 
                                                </div>
                                                {{ Form::select('grade', \App\Models\User::$grades, null, ['class' => 'form-control fix-select', 'id' => 'select-grade', 'placeholder' => __('choose_grade')]) }}
                                                <p> {{ $errors->first('grade')}} </p>
                                            </div>
                                        {{ Form::submit(__('update'), ['class' => 'btn btn-common submit', 'name' => 'update_info']) }}
                                        {{ Form::close() }}
                                    </div>
                                </div>
                            </div>
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h4 class="panel-title"> <a aria-expanded="false" class="collapsed" href="#collapseB2" data-toggle="collapse"> {{ __('change_password') }} </a> </h4>
                                </div>
                                <div aria-expanded="false" class="panel-collapse collapse" id="collapseB2">
                                    <div class="panel-body">
                                        ...
                                        {{ Form::model($selectedUser, ['route' => ['users.update', $selectedUser->id]]) }}
                                            <input name="_method" type="hidden" value="PUT">
                                            <div class="form-group">
                                                {{ Form::label('old_password', __('old_password'), ['class' => 'control-label']) }}
                                            ...
                                            <div class="form-group">
                                                {{ Form::label('new_password', __('new_password'), ['class' => 'control-label']) }}
                                            ...
                                            <div class="form-group">
                                                {{ Form::label('password_confirmation', __('retype_new_password'), ['class' => 'control-label']) }}
                                                <div>
                                                    {{ Form::password('password_confirmation', ['class' => 'form-control', 'id' => 'password-confirmation']) }}
                                                </div>
                                                <p> {{ $errors->first('password_confirmation')}} </p>
                                            </div>
                                        {{ Form::submit(__('update_password'), ['class' => 'btn btn-common submit', 'name' => 'update_password']) }}
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#address').focus(function() {
                $('#address').parent().removeClass('error-input');
        ...
        $('select#commune').change(function(){
            ...
            var working_place = province + ' province, ' + district + ' district, ' + commune + ' commune.';
